...
class AppAuthenticator extends AbstractLoginFormAuthenticator
{
    ...
    public function authenticate(Request $request): Passport
    {
        ...
        if (empty($participant)) {
            throw new CustomUserMessageAuthenticationException('Pseudo ou email invalide. Il est possible que votre compte soit désactivé, veuillez contacter un administrateur.');
        }

        return new Passport(
            new UserBadge($usernameOrEmail, function () use ($participant) {
                return $participant[0]; // Assuming the first participant is the desired one
            }),
